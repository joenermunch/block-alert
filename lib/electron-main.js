...
app.whenReady().then(async () => {
  ...
  const display = screen.getDisplayMatching(state.bounds || screen.getPrimaryDisplay().workArea);
  const defaultSize = initialCompact ? COMPACT_SIZE : FULL_SIZE;
  const defaultBounds = {
    x: display.workArea.x + display.workArea.width - defaultSize.width - 16,
    y: display.workArea.y + display.workArea.height - defaultSize.height - 24,
    ...defaultSize,
  };
