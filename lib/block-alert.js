...
export function macCommand(title, message, duration, keepOpen = false, state = 'blocked', compact = false, muted = false) {
  ...
  return `block-alert --title ${shellQuote(title)} --message ${shellQuote(message)} --duration ${shellQuote(String(duration))} --state ${shellQuote(state)}${compactFlag}${muteFlag}${keepOpenFlag}`;
}
...
function run(argv) {
  ...
  var muted = argv[7] === '--mute' || defaults.boolForKey($('block-alert.sound-muted'));
  ...
  panel.setFrameAutosaveName($('block-alert.pet-frame-v4'));
  panel.setFrameUsingName($('block-alert.pet-frame-v4'));
  ...
  function changeMode(nextCompact) {
    ...
    layout();
    panel.saveFrameUsingName($('block-alert.pet-frame-v4'));
  }
  ...
  ObjC.registerSubclass({
    ...
    methods: {
      ...
      'toggleMute:': {
        ...
        implementation: function () {
          muted = !muted;
          defaults.setBoolForKey(muted, $('block-alert.sound-muted'));
  ...
  const contextMenu = $.NSMenu.alloc.initWithTitle($('block-alert'));
...
export async function main(argv, { platform = process.platform, runner = spawnSync, output = console.log } = {}) {
  ...
  if (options.help) {
    output('Usage: block-alert [--state working|blocked] [--compact] [--mute] [--duration 2..30] [--keep-open] [--relay-mac] [--dry-run]');
    return;
  }
  if (options.dryRun) {
